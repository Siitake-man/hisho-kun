...
def test_01_storage_package_phase2_exports():
    ...
    for symbol in expected_symbols:
        assert hasattr(storage, symbol), f"storage に {symbol} が存在しません"


def test_02_database_and_audit_logger_facade_identity():
    ...
    for name, storage_obj, target_obj in check_pairs:
        assert storage_obj is target_obj, f"{name} の参照先が一致しません (Facade不整合)"


def test_03_audit_repo_direct_usage():
    ...
    try:
        ...
        assert len(logs) == 1
        assert logs[0].request_id == "req_phase2_01"
        assert logs[0].command == "pytest tests/"
    finally:
        if os.path.exists(temp_db):
            os.remove(temp_db)


def test_04_device_repo_direct_usage():
    ...
    try:
        ...
        assert device.device_name == "Boss iPhone 15"
        assert device.is_revoked == 0
        ...
        assert synced_device.ip_address == "10.0.0.1"

        # 4. 一覧取得
        all_devs = get_all_devices(db_path=temp_db)
        assert len(all_devs) == 1
        assert all_devs[0].device_name == "Boss iPhone 15"
        ...
        assert revoked_device.is_revoked == 1
    finally:
        if os.path.exists(temp_db):
            os.remove(temp_db)
