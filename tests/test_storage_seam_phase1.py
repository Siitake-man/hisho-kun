...
def test_01_storage_package_exports():
...
def test_02_database_facade_backward_compatibility():
    ...
    for name, storage_obj, database_obj in check_pairs:
        assert storage_obj is database_obj, f"{name} の参照先が一致しません (Facade不整合)"


def test_03_init_db_in_temp_database():
    ...
    try:
        ...
        assert not missing, f"必須テーブルが作成されていません: {missing}"
    finally:
        if os.path.exists(temp_db):
            os.remove(temp_db)


def test_04_model_validation_integrity():
    ...
    assert ev.title == "正常テスト"

    # Task のバリデーション
    task = storage.Task(title="タスクテスト", status="todo")
    assert task.title == "タスクテスト"
