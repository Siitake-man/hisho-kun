...
def get_google_auth_status() -> Dict[str, Any]:
    ...
    if not has_token:
        return {
            ...
            "email": "",
            "message": "未認証 (credentials.json を設定してログインしてください)" if has_creds else "credentials.json 未配置"
        }
    ...
    try:
        ...
        if creds and creds.valid:
            ...
            try:
                ...
                email = os.getenv("GOOGLE_CALENDAR_ID", "Google アカウント")
                
            return {
                "authenticated": True,
                "has_credentials": has_creds,
                "email": email,
                "message": f"✓ 連携済み ({email})"
            }
    except Exception as e:
        logger.warning(f"Google 認証状態チェックエラー: {e}")
...
def run_google_oauth_flow(custom_creds_path: Optional[str] = None) -> Tuple[bool, str]:
    """
    Google OAuth2 ブラウザ同意画面を起動して認証を完了し token.json を保存
    
    Args:
        custom_creds_path: ユーザーが指定した credentials.json のパス
        
    Returns:
        Tuple[bool, str]: (成功フラグ, メッセージ/メールアドレス)
    """
    try:
        from google_auth_oauthlib.flow import InstalledAppFlow
        from googleapiclient.discovery import build
        
        target_creds = Path(custom_creds_path) if custom_creds_path else CREDENTIALS_PATH
        if not target_creds.exists():
            return False, f"credentials.json が見つかりません: {target_creds}"
            
        # 同意画面を起動して認証トークンを取得
        flow = InstalledAppFlow.from_client_secrets_file(str(target_creds), SCOPES)
        ...
        logger.error(f"Google OAuth認証フロー失敗: {e}")
        return False, str(e)


def revoke_google_auth() -> bool:
    """Google 認証トークンを削除してログアウト"""
    try:
        if TOKEN_PATH.exists():
            TOKEN_PATH.unlink()
        return True
    except Exception as e:
        logger.error(f"Google ログアウト失敗: {e}")
        return False
